...
export const NotificationProvider = ({ children }) => {
  ...
  const loadNotifications = async () => {
    try {
      ...
      console.log('🌐 URL da requisição:', `${api.defaults.baseURL}/notifications/`);
      ...
      // Verificar se a API retornou HTML ao invés de JSON
      if (typeof response.data === 'string' && response.data.includes('<!doctype html>')) {
        console.error('❌ API retornou HTML - problema de autenticação ou URL incorreta');
        console.error('🔍 Primeiros 100 chars da resposta:', response.data.substring(0, 100));
        setNotifications([]);
        setUnreadCount(0);
        return;
      }
